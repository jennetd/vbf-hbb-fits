...
void muoncr(bool pass, string year, bool log=true){
...
void draw(int index, bool pass, string cat, string year, bool log=true){
...
void draw_datafit(){
  ...
  for(int j=0; j<years.size(); j++){
    ...
    for(int i=0; i<6; i++){
      draw(i,0,"ggf",years.at(j),0);
      draw(i,1,"ggf",years.at(j),0);
    }

    draw(0,0,"vbflo",years.at(j),0);
    draw(0,1,"vbflo",years.at(j),0);
    draw(1,0,"vbfhi",years.at(j),0);
    draw(1,1,"vbfhi",years.at(j),0);

    muoncr(0,years.at(j),0);
    muoncr(1,years.at(j),0);
  }
